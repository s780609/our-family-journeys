...
function SpotPhoto({ image, photo, title }: { image?: string; photo?: PhotoRef; title: string }) {
  const raw = image ?? photo?.src;
  const src = raw ? encodeURI(raw) : undefined;
  if (src) {
    return (
    ...
    <>
      ...
      <Card>
        <div className="grid grid-cols-[200px_1fr] gap-5 max-md:grid-cols-1">
          <SpotPhoto image={s.image} photo={s.photo} title={s.title} />
