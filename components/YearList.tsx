...
export function YearList({
  ...
  return (
    <aside className="bg-[var(--card-bg)] border border-[var(--rule)] rounded-2xl p-6 shadow-[var(--shadow-soft)] h-fit sticky top-8">
      <div className="flex items-end justify-between mb-5">
        <div className="font-[family-name:var(--font-hand)] text-[22px] text-[var(--ink-faint)]">
          — 年份清單 —
        </div>
            ...
            <div key={y} className="border-b border-dashed border-[var(--rule)] last:border-b-0">
              ...
              >
                <div className="flex items-baseline gap-3">
                  <span className="font-serif font-black text-[32px] text-[var(--ink)] leading-none">
                    {y}
                  </span>
                ...
                <div className="pb-3 pl-1 flex flex-col gap-2.5">
                  {trips.map((t) => (
                    <Link
                      key={t.slug}
                      href={`/trips/${t.slug}`}
                      className="group flex items-center justify-between gap-3 p-3 rounded-lg bg-[var(--paper)] hover:bg-[var(--coral)] hover:text-white transition-all no-underline"
                    >
                      <div className="min-w-0">
                        <div className="font-serif font-bold text-[15px] text-[var(--ink)] group-hover:text-white truncate">
                          {t.title}
                        </div>
                        <div className="text-xs text-[var(--ink-faint)] group-hover:text-white/80">
                          {t.dateRangeLabel} · {t.location}
                        </div>
                      </div>
                      <span className="text-[var(--ink-faint)] group-hover:text-white">→</span>
                    </Link>
                  ))}
                </div>
              )}
      ...
      <div className="mt-6 pt-5 border-t border-dashed border-[var(--rule)]">
        <div className="font-[family-name:var(--font-hand)] text-[var(--ink-faint)] text-sm">
          新增旅程 → 在 <code className="text-[var(--coral)]">content/trips/</code> 建立 .md 檔案
        </div>
      </div>
    </aside>
  );
